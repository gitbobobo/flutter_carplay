...
class FlutterCarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate {
    ...
    static public func updateNowPlayingConfig(_ nowPlayingTemplate: FCPNowPlayingTemplate) {
        let nowPlayingInstance = CPNowPlayingTemplate.shared
        
        nowPlayingInstance.isAlbumArtistButtonEnabled = nowPlayingTemplate.isAlbumArtistButtonEnabled
        nowPlayingInstance.isUpNextButtonEnabled = nowPlayingTemplate.isUpNextButtonEnabled
        if nowPlayingTemplate.upNextTitle != nil {
            nowPlayingInstance.upNextTitle = nowPlayingTemplate.upNextTitle!
        }
        
        nowPlayingInstance.updateNowPlayingButtons(nowPlayingTemplate.nowPlayingButtons.map { $0.get })
        
        if nowPlayingObserver == nil {
            nowPlayingObserver = FCPNowPlayingTemplateObserver()
            nowPlayingInstance.add(nowPlayingObserver!)
        }
    }
...
class FCPNowPlayingTemplateObserver: NSObject, CPNowPlayingTemplateObserver {
    func nowPlayingTemplateUpNextButtonTapped(_ nowPlayingTemplate: CPNowPlayingTemplate) {
        FCPStreamHandlerPlugin.sendEvent(type: FCPChannelTypes.onNowPlayingButtonPressed,
                                         data: ["btnType": FCPNowPlayingButtonTypes.upNext])
    }
    
    func nowPlayingTemplateAlbumArtistButtonTapped(_ nowPlayingTemplate: CPNowPlayingTemplate) {
        FCPStreamHandlerPlugin.sendEvent(type: FCPChannelTypes.onNowPlayingButtonPressed,
                                         data: ["btnType": FCPNowPlayingButtonTypes.albumArtist])
    }
}
