...
public class SwiftFlutterCarplayPlugin: NSObject, FlutterPlugin {
    ...
    public static var rootTemplate: CPTemplate? {
        get {
            if (_rootTemplate == nil) {
                let emptyTemplate = CPListTemplate(title: "", sections: [])
                emptyTemplate.emptyViewTitleVariants = ["📲"]
                emptyTemplate.emptyViewSubtitleVariants = ["Waiting for Connection..."]
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case FCPChannelTypes.showNowPlaying:
            ...
            break
        case FCPChannelTypes.updateNowPlayingConfig:
            guard let args = call.arguments as? [String : Any] else {
                result(false)
                return
            }
            let nowPlayingTemplate = FCPNowPlayingTemplate.init(obj: args)
            FlutterCarPlaySceneDelegate.updateNowPlayingConfig(nowPlayingTemplate)
            result(true)
            break
